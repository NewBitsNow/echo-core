...
def check_git_status() -> dict:
    ...
    # Branch
    status["branch"] = run_git(REPO_DIR, "branch", "--show-current")

    # Uncommitted changes
    status["has_uncommitted"] = False
    status["uncommitted_files"] = []
    dirty = run_git(REPO_DIR, "status", "--porcelain")
    if dirty:
    ...
    try:
        ...
        status["ahead_behind"] = run_git(REPO_DIR, "rev-list", "--left-right", "--count", "HEAD...@{upstream}")
    except (subprocess.TimeoutExpired, subprocess.CalledProcessError):
        status["ahead_behind"] = "unknown"

    # Recent commits
    log = run_git(REPO_DIR, "log", "--oneline", "-5")
...
def check_twin_output() -> dict:
    """Check the output directory for changes."""
    info = {}
...
def check_disk_usage() -> dict:
    """Check disk usage of the working directory."""
    stat = os.statvfs(str(TWIN_OUTPUT_DIR))
